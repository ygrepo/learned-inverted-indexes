...
from segmentation import ClusterFinder
...
def save_weights_and_deltas(logger, device, batch, model, scrap):
    ...
    #log_model(model)
    model_lossy_compression = decompress(compressed_weights, bias=True, scrap=scrap).to(device)
...
def train_posting_lists(device, delta_freqs_file, delta_docs_file, deltas_path, logger, posting_lists_dataset,
                        lr=0.01, batch_size=32, stop_threshold=1e-3, scrap_threshold=100,
    ...
    for posting_list, pl_length in posting_lists_loader:
        cluster_finder = ClusterFinder(posting_list, batch_size/5)
        cluster_finder.segment()

        all_deltas += train(device=device, delta_freqs_file=delta_freqs_file,
                            delta_docs_file=delta_docs_file,
...
def train(device, delta_freqs_file, delta_docs_file, cluster_finder,
          lr=0.01, stop_threshold=1e-3, scrap_threshold=100,
    ...
    batch_idx = 0
    total_batches = cluster_finder.get_number_batches()
    for data, indexes in cluster_finder:

        batch_size = data.size()[1]

        model = ShallowNetwork(3, True).to(device)
        lambda_l2 = 1e-5
        ...
        # print("data={}".format(data.view(1,-1)))
        # print("indexes={}".format(indexes.view(1,-1)))
        ...
        for epoch in range(1, epochs + 1):
            ...
            if loss.item() < stop_threshold:
                ...
                # logger.info('Loss is less that theshold: {:5.2f} | Batch/Total batches: {:3d}/{:3d} | batch size: {:5.2f} |'
                #             ' End of epoch: {:3d} | time: {:5.2f}s | loss {:5.3f} '
                #             .format(stop_threshold, batch_idx, total_batches, batch_size, epoch, (time.time() - start_time), loss.item()))
                #plot_errors(predictions, data)
                deltas_batch = save_weights_and_deltas(logger, device, (indexes, data), model, scrap=False)
            ...
            if epoch % log_interval == 0:
                logger.info('-' * 89)
                logger.info(
                    '| Batch/Total batches: {:3d}/{:3d} | batch size: {:5.2f} | End of epoch: {:3d} | time: {:5.2f}s | loss {:5.3f} '
                        .format(batch_idx, total_batches, batch_size, epoch, (time.time() - start_time), loss.item()))
                logger.info('-' * 89)
                start_time = time.time()
        ...
        batch_idx += 1
        deltas_batch = save_weights_and_deltas(logger, device, (indexes, data), model,
...
SEED:{SEED:5d} | LOG_INTERVAL:{LOG_INTERVAL:5d}")
    ...
    delta_freqs_file = os.path.join(DELTA_DIR, "b{}_e{}_deltas.freqs".format(BATCH_SIZE, EPOCHS))
    delta_docs_file = os.path.join(DELTA_DIR, "b{}_e{}_deltas.docs".format(BATCH_SIZE, EPOCHS))
    ...
    train_posting_lists(device=DEVICE, delta_freqs_file=delta_freqs_binfile, delta_docs_file=delta_docs_binfile,
                        ...
                        lr=LR, batch_size=BATCH_SIZE, stop_threshold=STOP_THRESHOLD, scrap_threshold=SCRAP_THRESHOLD,
